...
class Member(RulesModel):
    ...
    emergency_phone_primary = PhoneNumberField(
        verbose_name=_("first emergency phone"), blank=True, null=True
    )
    emergency_phone_secondary = PhoneNumberField(
        verbose_name=_("second emergency phone"), blank=True, null=True
    )
    ...
    password_change_required = models.BooleanField(
        ...
        default=False,
        help_text=_(
            "If flagged signals that this users will need to reset their password at the next login."
        ),
    )

    is_organization_admin = models.BooleanField(
        _("Organization admin"),
        default=False,
        help_text=_(
            "An organization admin will have advanced access rights into the system."
        ),
    )
    ...
    def delete(
        self, using: Any = DEFAULT_DB_ALIAS, keep_parents: bool = False
    ...
    def create_member(
        ...
        try:
            ...
            if users.count() == 1:
                if (
                    hasattr(users.first(), "member")
                    and users.first().member is not None
                ):
